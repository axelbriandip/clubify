...
export default function MatchesList({ initialMatches, clubSlug, clubName }: MatchesListProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Selector de Filtros */}
      <div className="flex gap-1.5 bg-slate-100/80 border border-slate-200/50 p-1 rounded-xl">
        {[
          ...
          return (
            <button
              ...
              className={`flex-1 py-2 text-[10px] font-black rounded-lg uppercase tracking-wider transition-all duration-200 ${
                isActive
              ...
              >
                <div className="space-y-3.5">
                  ...
                  <div className="flex justify-between items-center text-sm font-black text-slate-900 py-1">
                    <span className={`w-[40%] truncate ${match.homeTeam.isOwnClub ? "text-[var(--primary-club)] font-black" : "text-slate-800"}`}>
                      {match.homeTeam.name}
                    </span>
                    ...
                    <span className={`w-[40%] text-right truncate ${match.awayTeam.isOwnClub ? "text-[var(--primary-club)] font-black" : "text-slate-800"}`}>
                      {match.awayTeam.name}
                    </span>
                  </div>

                  {/* Detalles adicionales */}
                  <div className="flex flex-wrap gap-x-4 gap-y-1.5 pt-1.5 border-t border-slate-50">
                    {match.matchTime && (
                ...
                <div className="border-t border-slate-100 pt-3 flex justify-end">
                  <button
                    ...
                    className="inline-flex items-center text-[10px] font-bold text-slate-500 hover:text-[var(--primary-club)] bg-slate-50 hover:bg-[var(--primary-club)]/5 border border-slate-200/60 hover:border-[var(--primary-club)]/20 px-3 py-1.5 rounded-full transition-all duration-200"
                  >
                    <Share2 className="h-3.5 w-3.5 mr-1 text-slate-400 hover:text-inherit" />
                    Compartir WhatsApp
                  </button>
                </div>
              </div>
