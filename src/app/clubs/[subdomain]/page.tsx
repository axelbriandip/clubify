...
export default async function ClubPublicPage({ params }: ClubPublicPageProps) {
  ...
  return (
    <div 
      className="bg-slate-50 text-slate-800 font-outfit"
      style={{
        "--primary-club": primaryColor,
        "--secondary-club": secondaryColor,
      } as React.CSSProperties}
    >
      {/* ---------------- SECCIÓN HERO / PORTADA PREMIUM CLARA ---------------- */}
      <section className="relative min-h-[60vh] flex items-center justify-center py-20 px-6 overflow-hidden border-b border-slate-100">
        ...
        {club.heroImages.length > 0 ? (
          ...
          >
            {/* Overlay Oscuro con Gradiente para Legibilidad del texto blanco */}
            <div className="absolute inset-0 bg-gradient-to-r from-slate-950/80 via-slate-950/65 to-slate-950/80"></div>
          </div>
        ) : (
          <div className="absolute inset-0 bg-gradient-to-tr from-[var(--secondary-club)]/5 via-[var(--primary-club)]/10 to-[var(--secondary-club)]/5">
        ...
        <div className="relative max-w-4xl w-full mx-auto text-center space-y-8 z-10">
          ...
          <div className="pt-4">
            <Link
              ...
              className="inline-flex items-center justify-center px-8 py-3.5 border border-transparent text-xs font-black uppercase tracking-wider rounded-full text-white hover:brightness-110 shadow-lg shadow-[var(--primary-club)]/20 hover:shadow-[var(--primary-club)]/35 hover:-translate-y-0.5 hover:scale-103 transition-all duration-300 cursor-pointer"
            >
              Hacete Socio
            </Link>
          </div>
          ...
          {club.news.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {club.news.map((item) => (
                <div 
                  key={item.id} 
                  className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:border-[var(--primary-club)]/20 transition-all duration-300 group flex flex-col justify-between"
                >
                  <div>
                    {item.imageUrl && (
                  ...
                  <div className="p-6 pt-0 border-t border-slate-100/60 mt-4 flex justify-between items-center text-xs">
      ...
      <section className="bg-slate-100/70 py-20 border-t border-b border-slate-200/50">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div>
              ...
              <h2 className="font-outfit font-black text-3xl uppercase tracking-tight text-slate-900 mt-1">
                Contacto Oficial
              </h2>
            </div>
              ...
              {club.settings?.contactEmail && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-semibold text-slate-805">{club.settings.contactEmail}</span>
                  </div>
                </div>
              )}
              {club.settings?.contactPhone && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-semibold text-slate-805">{club.settings.contactPhone}</span>
                  </div>
                </div>
              )}
              {club.settings?.addressText && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-semibold text-slate-805 leading-snug">{club.settings.addressText}</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <section className="py-20 px-6 border-b border-slate-200/40 bg-white">
          ...
          <div className="max-w-7xl mx-auto px-6 space-y-10">
            ...
            <div className="flex flex-wrap justify-center items-center gap-10 md:gap-16">
              {club.sponsors.map((sponsor) => (
                <div 
                  key={sponsor.id} 
                  className="h-10 md:h-12 flex items-center justify-center opacity-70 hover:opacity-100 transition-all duration-300 transform hover:scale-102"
                >
                  <img
