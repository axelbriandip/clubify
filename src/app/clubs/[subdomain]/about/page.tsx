      ...
      <main className="max-w-5xl mx-auto px-6 py-16 space-y-20">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
          <div className="md:col-span-2 space-y-6">
            <div className="flex items-center gap-3 border-b border-slate-200 pb-4">
              <div className="p-2.5 rounded-2xl bg-[var(--primary-club)]/10 text-[var(--primary-club)]">
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-black text-sm uppercase tracking-wider text-slate-900 border-b border-slate-100 pb-2">
              Ficha del Club
            </h3>
            <ul className="space-y-3 text-xs">
              <li className="flex justify-between">
                <span className="text-slate-400 font-bold">Nombre Oficial</span>
                <span className="font-extrabold text-slate-805">{club.name}</span>
              </li>
              <li className="flex justify-between">
                <span className="text-slate-400 font-bold">Lema Oficial</span>
                <span className="font-semibold text-slate-700 italic">"{club.settings?.heroSubtitle || "Fomentando el Deporte"}"</span>
              </li>
              <li className="flex justify-between">
                <span className="text-slate-400 font-bold">Fundación</span>
                <span className="font-extrabold text-slate-805">15 de Octubre de 1910</span>
              </li>
              <li className="flex justify-between">
                <span className="text-slate-400 font-bold">Colores</span>
                <span className="font-extrabold text-slate-805">Aurinegro</span>
              </li>
              <li className="flex justify-between">
                <span className="text-slate-400 font-bold">Apodo</span>
                <span className="font-extrabold text-slate-805">El Aurinegro</span>
              </li>
            </ul>
          </div>
        </section>
        ...
        <section className="space-y-8">
          ...
          {club.boardMembers.length === 0 ? (
            <p className="text-xs text-slate-400 italic">No hay autoridades institucionales asignadas actualmente.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {club.boardMembers.map((member) => (
                <div 
                  key={member.id}
                  className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow duration-300"
                >
                  <div className="h-14 w-14 rounded-full bg-slate-900 flex items-center justify-center text-white font-black text-xl border-2 border-[var(--primary-club)] shrink-0">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-black uppercase text-[var(--primary-club)] bg-[var(--primary-club)]/5 px-2 py-0.5 rounded border border-[var(--primary-club)]/10 mt-1 inline-block">
                      {member.position}
                    </span>
                    {member.termStart && (
        ...
        <section className="space-y-8">
          ...
          {club.facilities.length === 0 ? (
            <p className="text-xs text-slate-400 italic">No hay instalaciones registradas en el portal del club.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {club.facilities.map((fac) => (
                <div 
                  key={fac.id}
                  className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
                >
                  <div>
                    {fac.imageUrl && (
                      <div className="h-44 bg-cover bg-center overflow-hidden">
                        <img 
                          src={fac.imageUrl} 
                          alt={fac.name} 
                    ...
                    <div className="p-6 space-y-2">
                      <h3 className="font-black text-lg text-slate-950">{fac.name}</h3>
                      <p className="text-xs text-slate-500 leading-relaxed line-clamp-4">{fac.description}</p>
                    </div>
                  </div>

                  <div className="p-6 pt-0 mt-4 border-t border-slate-100/50 flex items-center gap-2 text-xs text-slate-400">
                    <MapPin className="h-4.5 w-4.5 text-[var(--primary-club)] shrink-0" />
                    <span className="font-bold leading-normal truncate">{fac.address}</span>
                  </div>
                </div>
              ))}
            </div>
