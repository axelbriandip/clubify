  ...
  return (
    ...
    >
      ...
      <header className="bg-slate-900 text-white py-14 px-6 relative overflow-hidden">
        ...
        {/* Degradado oscuro para que el texto sea perfectamente legible */}
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 space-y-16">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-black text-sm uppercase tracking-wider text-slate-800 border-b border-slate-100 pb-2">
              Ficha del Club
            </h3>
          ...
          {club.boardMembers.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {club.boardMembers.map((member) => (
                <div 
                  key={member.id}
                  className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center gap-4 hover:shadow-md hover:border-[var(--primary-club)]/30 transition-all duration-300"
                >
          ...
          {club.facilities.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {club.facilities.map((fac) => (
                <div 
                  key={fac.id}
                  className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-md hover:border-[var(--primary-club)]/30 transition-all duration-300 flex flex-col justify-between"
                >
                  <div>
                    {fac.imageUrl && (
                    ...
                    <div className="p-6 space-y-2">
                      <h3 className="font-black text-lg text-slate-900">{fac.name}</h3>
