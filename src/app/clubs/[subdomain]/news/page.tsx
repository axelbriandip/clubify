        ...
        <div className="max-w-5xl mx-auto px-6 flex gap-8">
          <Link
            ...
            className={`flex items-center py-4 text-sm font-black uppercase tracking-wider border-b-4 transition-colors ${
              tab === "news" ? "border-[var(--primary-club)] text-slate-900" : "border-transparent text-slate-400 hover:text-slate-700"
            }`}
          >
          ...
          <Link
            href={`?tab=albums`}
            className={`flex items-center py-4 text-sm font-black uppercase tracking-wider border-b-4 transition-colors ${
              tab === "albums" ? "border-[var(--primary-club)] text-slate-900" : "border-transparent text-slate-400 hover:text-slate-700"
            }`}
          >
                  ...
                  >
                    <div>
                      ...
                      <div className="p-6 space-y-3">
                        ...
                        <h3 className="font-black text-xl text-slate-900 leading-snug group-hover:text-[var(--primary-club)] transition-colors">
                          {item.title}
                        </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {club.photoAlbums.map((album) => (
                  <Link
                    ...
                    key={album.id}
                    className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:border-[var(--primary-club)]/20 hover:-translate-y-0.5 transition-all duration-300 flex flex-col justify-between group cursor-pointer"
                  >
                    <div>
                      {/* Portada */}
                      <div 
                      ...
                      <div className="p-6 space-y-2">
                        <h3 className="text-lg font-black text-slate-900 group-hover:text-[var(--primary-club)] transition-colors truncate">{album.title}</h3>
                        <p className="text-xs text-slate-505 line-clamp-3 leading-relaxed">{album.description || "Explora las fotos de este álbum oficial."}</p>
                      </div>
                    </div>

                    <div className="p-6 pt-0 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-slate-405 mt-4">
                      <span>Publicado</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col text-slate-800">
            ...
            <div className="p-6 border-t border-slate-100 flex justify-between items-center bg-slate-50/40 text-xs font-bold text-slate-400">
              ...
              <Link
                href={`?tab=news`}
                className="px-5 py-2.5 bg-slate-900 text-white rounded-full hover:bg-slate-800 transition-colors uppercase tracking-wider text-[10px] font-black"
              >
                Cerrar Lector
              </Link>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col">
              ...
              {selectedAlbum.photos.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {selectedAlbum.photos.map((photo: any) => (
                    <div 
                      key={photo.id}
                      className="bg-slate-950 border border-slate-850 rounded-2xl overflow-hidden shadow-md"
                    >
            ...
            <div className="p-6 border-t border-slate-800 flex justify-between items-center bg-slate-950/60 text-xs font-bold text-slate-450">
              <span className="flex items-center text-[var(--primary-club)] font-black uppercase tracking-widest text-[10px]">
                <Heart className="h-3.5 w-3.5 mr-1 fill-[var(--primary-club)]" /> Olimpo Multimedia
              </span>
              <Link
                href={`?tab=albums`}
                className="px-5 py-2.5 bg-[var(--primary-club)] text-white rounded-full hover:brightness-110 transition-all uppercase tracking-wider text-[10px] font-black"
              >
                Volver a Álbumes
              </Link>
            </div>
