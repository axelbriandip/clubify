...
export default async function ClubNewsPage({ params, searchParams }: NewsPageProps) {
  ...
  return (
    ...
    >
      ...
      {/* Tabs Selector de Secciones (Estilo Oscuro) */}
      <div className="bg-slate-950 border-b border-slate-900 sticky top-20 z-40 shadow-xl">
        <div className="max-w-5xl mx-auto px-6 flex gap-8">
          <Link
            ...
            className={`flex items-center py-4 text-sm font-black uppercase tracking-wider border-b-4 transition-colors ${
              tab === "news" ? "border-[var(--primary-club)] text-white" : "border-transparent text-slate-500 hover:text-slate-300"
            }`}
          >
          ...
          <Link
            href={`?tab=albums`}
            className={`flex items-center py-4 text-sm font-black uppercase tracking-wider border-b-4 transition-colors ${
              tab === "albums" ? "border-[var(--primary-club)] text-white" : "border-transparent text-slate-500 hover:text-slate-300"
            }`}
          >
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        {tab === "news" && (
          <div>
            {club.news.length === 0 ? (
              <div className="bg-slate-900/40 border border-slate-850 p-12 rounded-3xl text-center text-slate-500 text-sm shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {club.news.map((item) => (
                  <div 
                    key={item.id} 
                    className="bg-slate-900/60 border border-slate-850 rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-[var(--primary-club)]/10 hover:border-[var(--primary-club)]/40 transition-all duration-300 group flex flex-col justify-between"
                  >
                    <div>
                      {item.imageUrl && (
                      ...
                      <div className="p-6 space-y-3">
                        ...
                        <h3 className="font-black text-xl text-white leading-snug group-hover:text-[var(--primary-club)] transition-colors">
                          {item.title}
                        </h3>
                    ...
                    <div className="p-6 pt-0 text-right mt-4">
                      <Link 
        ...
        {tab === "albums" && (
          <div>
            {club.photoAlbums.length === 0 ? (
              <div className="bg-slate-900/40 border border-slate-850 p-12 rounded-3xl text-center text-slate-505 text-sm shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {club.photoAlbums.map((album) => (
                  <Link
                    ...
                    key={album.id}
                    className="bg-slate-900/60 border border-slate-850 rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-[var(--primary-club)]/10 hover:border-[var(--primary-club)]/40 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group cursor-pointer"
                  >
                    <div>
                      {/* Portada */}
                      <div 
                      ...
                      >
                        <div className="absolute top-4 right-4 bg-slate-950/85 backdrop-blur-sm text-blue-400 border border-slate-800 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg">
                          {album.photos?.length || 0} Fotos
                        </div>
                      </div>
                      <div className="p-6 space-y-2">
                        <h3 className="text-lg font-black text-white group-hover:text-[var(--primary-club)] transition-colors truncate">{album.title}</h3>
                        <p className="text-xs text-slate-400 line-clamp-3 leading-relaxed">{album.description || "Explora las fotos de este álbum oficial."}</p>
                      </div>
                    </div>

                    <div className="p-6 pt-0 border-t border-slate-850 flex items-center justify-between text-xs font-bold text-slate-500 mt-4">
                      <span>Publicado</span>
      ...
      {selectedNews && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-850 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col text-slate-100">
            {/* Header del Modal */}
            ...
            <div className="overflow-y-auto p-6 space-y-6 flex-grow">
              ...
              <p className="text-xs text-slate-450 font-bold leading-relaxed border-l-4 border-[var(--primary-club)] pl-3 italic">
                {selectedNews.summary}
              </p>
              <div className="text-sm text-slate-300 leading-relaxed whitespace-pre-line space-y-4 font-medium">
                {selectedNews.content}
              </div>
            </div>

            {/* Footer */}
            <div className="p-6 border-t border-slate-850 flex justify-between items-center bg-slate-950/40 text-xs font-bold text-slate-550">
              <span>© {club.name} Oficial</span>
              <Link
                href={`?tab=news`}
                className="px-5 py-2.5 bg-slate-950 text-white border border-slate-800 rounded-full hover:bg-slate-800 transition-colors uppercase tracking-wider text-[10px] font-black"
              >
                Cerrar Lector
              </Link>
            </div>
